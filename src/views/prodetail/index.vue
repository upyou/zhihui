  ...
  <div class="prodetail">
    ...
    <div class="comment">
      ...
      <div class="comment-list">
        <div class="comment-item" v-for="item in comment" :key="item.goods_id">
          <div class="top">
    ...
    <div class="footer">
      ...
      <div class="icon-cart" @click="$router.push('/cart')">
        <van-icon name="cart-o" :badge="cartTotal" />
        <span>购物车</span>
      </div>
...
import { getProdetails, getProdetailsComment } from '@/api/category'
...
export default {
  ...
  methods: {
    ...
    async addCart () {
      ...
      const re = await Postcart(this.goodsId, this.num, this.detail.skuList[0].goods_sku_id)
      this.$toast('加入购物车成功')
      this.showPannel = false
      this.cartTotal = re.data.cartTotal
    },
    async goPay () {
      if (this.login()) {
        return
  ...
  async created () {
    const re = await getProdetails(this.goodsId)
    this.detail = re.data.detail
    this.images = re.data.detail.goods_images
    console.log(re)
    const res = await getProdetailsComment(this.goodsId, 3)
